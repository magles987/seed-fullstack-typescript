...
export type TKeyDiccStrategyGeneratorsIdFn =
  keyof UtilGeneratorId["DICC_GENERATORS_ID"];
 ...
 * ...
 */
export class UtilGeneratorId {
  protected readonly DICC_GENERATORS_ID = {
    /**id completo con la libreria uuid v4 */
    df_uuid: ((option?: TOptionForUuidv4) => {
      option = this.util.isObject(option) ? option : {};
      let id = uuidv4();
      return id;
    ...
    df_autoincrement: ((option?: TOptionForAutoincrement) => {
      option = this.util.isObject(option) ? option : ({} as any);
      let { lastId } = option;
      lastId = this.util.isNumber(lastId) ? lastId : 0;
      let id = lastId + 1;
      return id;
    }) as TCustomGenerateIdFn,
    ...
    df_strRandom: ((option?: TOptionForStrRandom) => {
      option = this.util.isObject(option) ? option : ({} as any);
      let { len } = option;
      len = this.util.isNumber(len) ? len : 16; //16 es tamaño predefinido del id
      let id = "";
      const charts =
  ...
  private static UtilGeneratorId_instance: UtilGeneratorId;
  /**... */
  protected constructor() {
    this.util = TwinBeeModule.util;
  }
  /** @returns la instancia única de la clase*/
  public static getInstance(): UtilGeneratorId {
    UtilGeneratorId.UtilGeneratorId_instance =
      typeof UtilGeneratorId.UtilGeneratorId_instance === "object" &&
      UtilGeneratorId.UtilGeneratorId_instance !== null
        ? UtilGeneratorId.UtilGeneratorId_instance
        : new UtilGeneratorId();
    return UtilGeneratorId.UtilGeneratorId_instance;
  }
   ...
   */
  public buildIdByStrategy(
    keyStrategyOrFn: TKeyDiccStrategyGeneratorsIdFn | TCustomGenerateIdFn,
    option?: object
  ): any {
    let id: any;
    if (this.util.isFunction(keyStrategyOrFn)) {
      const fn = keyStrategyOrFn as TCustomGenerateIdFn;
      id = fn(); //función personalizada
    } else if (typeof keyStrategyOrFn === "string") {
      const fn = this.DICC_GENERATORS_ID[keyStrategyOrFn];
      id = fn(option);
      ...
      throw new Error(
        `${
          keyStrategyOrFn as any
        }is not strategy key or not function generator valid`
      );
    }
    return id;
  }
